...
import { ONLINE_LANGUAGE_QUIZ } from '../../constant/quiz-type'

import * as actions from '../../actions/online-language-quiz'

class OnlineLanguageQuizzes extends Component {
  constructor (props) {
    super(props)
    this.state = {
      page: 1,
  ...
  editorPage (quizId) {
    this.props.history.push(`/onlineLanguageQuizzes/${quizId}/editor`)
  }

  addQuiz () {
    this.props.history.push(`/onlineLanguageQuizzes/new`)
  }
  ...
  render () {
    const dataSource = this.props.onlineLanguageQuizzes.content.map(quiz => {
    ...
    const pagination = {
      ...
      total: this.props.onlineLanguageQuizzes.totalElements,
    ...
    return (
      <div>
        ...
        <QuizzesTable dataSource={dataSource} pagination={pagination}
          ...
          type={ONLINE_LANGUAGE_QUIZ}
        />
      </div>
    )
  }
}
const mapStateToProps = ({onlineLanguage}) => ({
  onlineLanguageQuizzes: onlineLanguage.quizzes
})
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(OnlineLanguageQuizzes))
